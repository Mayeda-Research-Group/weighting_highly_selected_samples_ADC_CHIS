...
#$ -j y 
#$ -l h_rt=4:00:00,h_data=4G,arch=intel-E5-*|intel-gold-* #requests 4 hours, 4GB of data (per core), specific intel nodes (required for twang package)
...
# TEST RUN:
##$ -t 1-3:1
# FULL RUN (length of the array of missing jobs):
#$ -t 1-11:1
## 
...
### (note: UPDATE line "#$ -t 1-2000:1" ACCORDINGLY!!!) 

array=(223 511 531 564 740 748 1042 1324 1363 1633 1680)

# SET THE SEED
###(note: array index starts at 0 so we need to scale back the current $SGE_TASK_ID)
myindex=${array[$SGE_TASK_ID-1]}

R CMD BATCH --no-save --no-restore "--args seed=$myindex "  hoffman_Analysis_18May2022.R output/output.$JOB_ID.$SGE_TASK_ID
