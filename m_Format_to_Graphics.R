...
for(race in c("all", unique(s_l_r_g$race_label))){
  ...
  # *******************************************************
  #
  # ---- **aTable 3. Standardized mean differences ----
  #
  # *******************************************************
  ...
  aTable3 <- left_join(u_r_l_g, results_s2, by="Covariate") %>% 
    mutate("Race" = race) 
  
  colnames(aTable3) <- c('Covariate',
                        'unweighted',
                        'raking weights', 
                        'sIOSW',
                        'sIOSW, algorithm: GBM',
  ...
  #remove SL and GBM results
  aTable3 %<>% dplyr::select("Covariate", "unweighted", "raking weights", 
                            "sIOSW", "Race")
  ...
  if(race == "all"){
    write.csv(aTable3, paste0(path_to_box, 
                              "KD_bootstrapping/Scripts_18May2022/Final graphics/Manuscript_aTable3a_021422_", 
                              race, ".csv"))
  } else{
    write.csv(aTable3, paste0(path_to_box, 
                              "KD_bootstrapping/Scripts_18May2022/Final graphics/Manuscript_aTable3b_021422_", 
                              race, ".csv"))
  }
...
#---- **stratified data ----
density$race <- factor(density$race, levels = c(1, 2, 3), 
...
#---- **comment out SL and GBM data ----
#add race column to overall results
summ_swr %<>% mutate("race" = "Overall") %>% dplyr::select("race", everything())

summ_swl %<>% mutate("race" = "Overall") %>% dplyr::select("race", everything())

#combine
Table2 <- t(rbind(summ_swr, summ_swr_race, summ_swl, summ_swl_race))

colnames(Table2) <- c(rep('Raking weights', 4), rep('sIOSW', 4))
# **************************************************************
#
...
#---- **comment out GMB and SL results and combine into one table ----
#add race_label to combined results
rescale_l_adc %<>% mutate("race_label" = "Overall") %>% 
  dplyr::select("race_label", everything())

rescale_l_chis %<>% mutate("race_label" = "Overall") %>% 
  dplyr::select("race_label", everything())

#combine into one table
aTable2 <- cbind(t(rbind(rescale_l_adc, rescale_l_chis)), 
                 t(rbind(rescale_l_adc_strat, rescale_l_chis_strat)))
...
write.csv(results, paste0(path_to_box, "KD_bootstrapping/Scripts_18May2022/Output/all_weights052522.csv"))

# For density plots
write.csv(density, paste0(path_to_box, "KD_bootstrapping/Scripts_18May2022/Output/weights_comb_short_0214222.csv"))
...
# ----*Manuscript Tables----
# Table 1.
write.csv(table1, paste0(path_to_box, "KD_bootstrapping/Scripts_18May2022/Final graphics/Manuscript_Table1_021422.csv"))

# # Table 2. Summary of stabilized ADC weights by raking, and logistic
# write.csv(table2, 
#           paste0(path_to_box, 
#                  "KD_bootstrapping/Scripts_18May2022/Final graphics/", 
#                  "Manuscript_Table2_021422.csv"))

# Table 2. Overall and Stratified by race, Summary of stabilized ADC weights by raking, logistic, GBM, and SL
write.csv(Table2, paste0(path_to_box, "KD_bootstrapping/Scripts_18May2022/Final graphics/Manuscript_Table2_021422.csv"))

# ----*Appendix Tables----
# aTable2. Distribution of rescaled propensity scores overall and by race
write.csv(aTable2, 
          paste0(path_to_box, "KD_bootstrapping/Scripts_18May2022/Final graphics/Manuscript_aTable2_021422.csv"))
